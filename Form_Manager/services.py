...
class FormManager:
    ...
    def check_what_is_empty(self, form_details_form):
        ask_for = []
        for field, value in form_details_form.dict().items():
            if value in [None, "", '', 0]:
    ...
    def extract_fields(self, query, ask_for):
        if "https://agente-terry.s3.amazonaws.com" in query:
            ...
            self.form.photographic_evidence.append(image_url)
            extracted_info = {"photographic_evidence": self.form.photographic_evidence}
    ...
    def ask_for_info(self, ask_for, thread):
        ...
        chat = self.client.beta.threads.messages.create(
            ...
            role="user", content="You are an assistant tasked with collecting information from the user to complete a form. "
    ...
    def handle_form(self, query, task, thread):
        ...
        try:
            ...
            new_details = FormDetails(**filtered_info)
            self.add_non_empty_details(self.form, new_details)
            ask_for = self.check_what_is_empty(self.form)
            ai_response = self.ask_for_info(ask_for, thread)

            if not ask_for:
                print("Formulario completo")
                ai_response = ( "All the necessary information has been gathered. Your intention to become a Terragene distributor has been forwarded to the appropriate department. "
                                "A representative will contact you shortly.")
                task.update_state('completed')

            task.set_response(ai_response)
        except Exception as e:
            raise RuntimeError(f"An error occurred while loading form: {str(e)}")

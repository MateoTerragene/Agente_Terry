document.addEventListener('DOMContentLoaded', function () {
    ...
    // Funcionalidad para el bot├│n de logout
  ...
  // Seleccionamos el ├¡cono de nuevo thread por su id
  const newThreadIcon = document.getElementById('new-thread-icon');
  if (newThreadIcon) {
      newThreadIcon.addEventListener('click', createNewThread);  // Llamar a la funci├│n cuando se haga clic
  }

  // Funci├│n para crear un nuevo thread
  async function createNewThread() {
        ...
        if (data.status === 'success') {
            console.log('Thread creado con ├®xito:', data.message);
        } else {
    ...
    async function sendQuery(prefixedQuery = '') {
        ...
        try {
            ...
            // Detectar espec├¡ficamente el error 504 de Nginx
            ...
            if (contentType?.includes('application/json')) {
                ...
                if (response.ok) {
                    ...
                    errorContainer.innerHTML = `ÔÜá´©Å Server Error: ${data.error || 'Unknown error'}`;
    ...
    startButton.addEventListener('click', function () {
        // Obtener el avatar seleccionado o asignar 'hand' si no hay selecci├│n
        ...
        fetch('/module_manager/set_avatar/', {
            ...
            .then(response => {
                if (response.ok) {
                    // Redirigir al chat despu├®s de seleccionar el avatar
